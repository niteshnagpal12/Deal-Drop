...
export async function sendAlertEmail(userEmail) {
  try {
    const { data, error } = await resend.emails.send({
      ...
      <body style="font-family: Arial, sans-serif; background-color: #f9f9f9; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #FA5D19 0%, #FF8C42 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 28px;">DealDrop Alert!</h1>
          </div>
          <div style="background-color: #fff; padding: 30px; border-radius: 0 0 10px 10px; text-align: center;">
            <p style="font-size: 18px; margin: 0; color: #333;">Hello ${
              userEmail.split("@")[0]
            }, this is to inform you that now we are available @ <a href="https://dealdrop.app" style="color: #FA5D19; text-decoration: none;">https://dealdrop.live</a></p>
      </body>
      </html>`,
    });
